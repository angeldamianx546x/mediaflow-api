...
public class UserController {
    ...
    @Operation(
        ...
        description = "Updates user information. Users can only update their own account unless they are ADMIN.",
    ...
    @Operation(
        ...
        description = "Permanently deletes a user. Users can only delete their own account unless they are ADMIN.",
        security = @SecurityRequirement(name = "bearerAuth")
    )
    @ApiResponses(value = {
    ...
    @Operation(
        ...
        description = "Returns the location of a user. Users can only access their own location unless they are ADMIN.",
        security = @SecurityRequirement(name = "bearerAuth")
    )
    @ApiResponses(value = {
        ...
        @ApiResponse(responseCode = "403", description = "Forbidden - Cannot access another user's location"),
        @ApiResponse(responseCode = "404", description = "User not found or has no assigned location")
    })
    @GetMapping("/{userId}/location")
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<?> getUserLocation(@PathVariable Integer userId) {
        // Validar que el usuario solo pueda ver su propia ubicación (o sea admin)
        if (!authenticationService.canAccess(userId)) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN)
                    .body(buildErrorResponse("No tienes permiso para ver esta ubicación"));
        }
        
        LocationResponse location = service.getUserLocation(userId);
    ...
    @Operation(
        ...
        description = "Returns user information by ID. Users can only access their own info unless they are ADMIN.",
        security = @SecurityRequirement(name = "bearerAuth")
    )
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "User info retrieved successfully"),
